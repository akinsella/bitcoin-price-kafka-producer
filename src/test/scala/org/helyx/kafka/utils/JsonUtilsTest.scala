...
import org.helyx.kafka.utils.JsonUtils.{fromJson, toJson}
...
class JsonUtilsTest extends FunSuite with Matchers {
  ...
  test("JSON body is correctly serialized") {
    ...
    val actualJsonQuote = toJson(quote, indent = true)

    val expectedJsonQuote = """{
       |  "time" : {
       |    "updatedISO" : "2018-02-02T16:48:00Z"
       |  },
       |  "disclaimer" : "This data was produced from the CoinDesk Bitcoin Price Index (USD). Non-USD currency data converted using hourly conversion rate from openexchangerates.org",
       |  "chartName" : "Bitcoin",
       |  "bpi" : {
    ...
    actualJsonQuote shouldEqual expectedJsonQuote
