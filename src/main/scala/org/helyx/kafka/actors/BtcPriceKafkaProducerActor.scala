...
import org.helyx.kafka.BitcoinPriceKafkaProducerMain.logger
...
class BtcPriceKafkaProducerActor(brokers:Seq[String], clientId:String, topic: String) extends Actor with ActorLogging {
  ...
  var producer:KafkaProducer[String, Quote] = _
  ...
  override def preStart():Unit = {
    ...
    props.put(VALUE_SERIALIZER_CLASS_CONFIG, classOf[KafkaAvroSerializer].getName)
    props.put("schema.registry.url", classOf[KafkaAvroSerializer].getName)

    producer = new KafkaProducer[String, Quote](props)
  }
  ...
  override def receive: Receive = {
    case quote: Quote =>
      ...
      producer.send(new ProducerRecord(topic, quote.time.updatedISO.toString, quote), new Callback {
